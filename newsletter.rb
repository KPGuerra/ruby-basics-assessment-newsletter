...
def calculate_recipients
  ...
  SUBSCRIBERS.reject { |email| (email == UNSUBSCRIBED[0]) || (email == UNSUBSCRIBED[1])}
end
...
def print_one_article(article)
  ...
  ARTICLES.find do |article_hash|
    if article_hash[:title] == article[:title]
      puts "\n#{article_hash[:title]} \nby: #{article_hash[:author]} \n#{article_hash[:text]}\n\n"
    end
  end 
end
